...
def coarsen_chain(
	...
	Args:
		...
		coarsen_inferior_limit: Minimum accepted coarse size. A new level is only kept if it
			still has at least this many nodes.
		coarsen_superior_limit: Optional maximum accepted coarse size. When provided,
			the returned coarse graph must have at most this many nodes.
	...
	"""
	inferior_limit = int(coarsen_inferior_limit)
	superior_limit = None if coarsen_superior_limit is None else int(coarsen_superior_limit)
	if inferior_limit < 1:
		raise ValueError("coarsen_inferior_limit must be >= 1")
	if superior_limit is not None and superior_limit < inferior_limit:
		raise ValueError("coarsen_superior_limit must be >= coarsen_inferior_limit")

	graphs = [H]
	maps: list[dict] = []
	initial_nodes = H.number_of_nodes()
	if superior_limit is not None and inferior_limit <= initial_nodes <= superior_limit:
		return graphs, maps

	stop_reason = None
	
	while graphs[-1].number_of_nodes() > inferior_limit and len(graphs) < max_levels:
		G_prev = graphs[-1]
		...
		if n_coarse < inferior_limit:
			# Do not accept a coarsening level that would overshoot below the requested lower bound.
			stop_reason = (
				f"the next level would undershoot the requested inferior limit "
				f"({n_coarse} < {inferior_limit})"
			)
			break

		if superior_limit is not None and n_coarse <= superior_limit:
			graphs.append(Gc)
			maps.append(label)
			return graphs, maps
		...
		if n_coarse >= n_prev:
			...
			stop_reason = f"the next level would not reduce the graph size ({n_coarse} >= {n_prev})"
			break
		...
		if reduction_ratio > coarsen_ratio:
			...
			stop_reason = (
				f"the next level would reduce too little "
				f"(ratio {reduction_ratio:.3f} > allowed {coarsen_ratio:.3f})"
			)
			break
	...
	final_nodes = graphs[-1].number_of_nodes()
	if inferior_limit <= final_nodes <= superior_limit:
		return graphs, maps

	reason_text = stop_reason or "the requested range could not be reached before hitting max_levels"
	raise ValueError(
		"Unable to coarsen graph into the requested node-count range "
		f"[{inferior_limit}, {superior_limit}]. "
		f"The closest accepted coarse graph has {final_nodes} nodes, because {reason_text}."
	)
