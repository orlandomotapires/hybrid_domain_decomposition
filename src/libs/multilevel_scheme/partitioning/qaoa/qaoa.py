...
def _qaoa_bipartition_on_qlm(
    ...
    *,
    ...
    objective_evaluation_counter = count(1)

    def objective(params: np.ndarray) -> float:
        evaluation_index = next(objective_evaluation_counter)
        ...
        result = submit_qlm_job(
            qpu,
            job,
            job_label=f"QAOA objective evaluation {evaluation_index} (n={n}, p={p})",
            timeout_seconds=qlm_job_timeout_seconds,
        )
    ...
    result = submit_qlm_job(
        qpu,
        circuit.to_job(nbshots=max(1, int(num_shots))),
        job_label=f"QAOA final sampling (n={n}, p={p}, shots={max(1, int(num_shots))})",
        timeout_seconds=qlm_job_timeout_seconds,
    )
