...
def partition_graph_metis(
    ...
    # Optional seed (may be ignored depending on pymetis version)
    # (Optional seed not passed to pymetis to avoid API mismatches)

    # Optional node weights (vweight)
    has_vw = False
    vwgt_f: List[float] = []
    for u in nodes:
...
# Backwards-compatible alias (some notebooks/scripts import this name)
def partitioning_graph_metis(
    G: nx.Graph,
    nparts: int = 2,
    weight: str = "weight",
    seed: int | None = None,
    ...
    return partition_graph_metis(G=G, nparts=nparts, weight=weight, seed=seed, verbose=verbose)
